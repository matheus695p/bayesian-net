...
def counterfactuals_barplot(df, col="n_eventos_stopped", letter_size=40):
    """
    Entrega un barplot por turnos de acuerdo a la columna que se entregue
    como argumento en la función
    Parameters
    ----------
    df : pandas.dataframe
        agrupación en una dataframe de la metrica que se busca analizar.
    col : string
        Nombre de la columna.
    letter_size : int, optional
        Tamaño de las letras. The default is 30.
    Returns
    -------
    Plot de matplotlib.pyplot.
    """
    plt.style.use('dark_background')
    cols = ["numero_ciclos"]
    for columna in df.columns:
        if col in columna:
            ...
            if "5" in columna:
                columna2 = columna
            cols.append(columna)
            nombre = columna.replace("_1", "").replace(
                "_5", "").replace("_", " ")
    df = df[cols]

    # separacion entre dia y noche
    labels = ["Muy pocos ciclos", "Baja cantidad de ciclos",
              "Cantidad Intermedia", "Alta cantidad de ciclos", "Muchos Ciclos"]

    day_means = list(df[columna1])
    night_means = list(df[columna2])
    # Obtenemos la posicion de cada etiqueta en el eje de X
    x = np.arange(len(labels))
    # tamaño de cada barra
    width = 0.3
    fig, ax = plt.subplots(1, figsize=(36, 17))
    # fig, ax = plt.subplots()
    # Generamos las barras para el conjunto de hombres
    rects1 = ax.bar(x - width/2, day_means, width, color="aquamarine",
                    label="Muy bajos " + nombre)
    # Generamos las barras para el conjunto de mujeres
    rects2 = ax.bar(x + width/2, night_means, width, color="navy",
                    label="Muy altos " + nombre)
    # Añadimos las etiquetas de identificacion de valores en el grafico
    ylabel = nombre
    title = "Counterfactual: P (numero de ciclos | " + ylabel + ")"
    ax.set_ylabel(ylabel, fontsize=letter_size+10)
    ax.set_title(title,
                 fontsize=letter_size+10)
    ax.tick_params(axis='both', which='major', labelsize=letter_size+2)
    plt.legend([columna1, columna2],
               loc='upper right',
               prop={'size': letter_size+5})
    plt.xticks(x, labels, fontsize=letter_size)
    plt.xticks(rotation=75)
    plt.legend(frameon=False, fontsize=letter_size)
    plt.show()

    path = f"results/counterfactuals/{nombre}.png"
    fig.savefig(path)
